...
void ReceiveUpdate()
{
	...
	{
		PacketLength = Eth_updateComm.parsePacket();
		if (PacketLength > 0)
		{
			if (PacketLength > 500) PacketLength = 500;
			Eth_updateComm.read(ReceivedData, PacketLength);
			if (UpdateMode)
			{
			...
			else
			{
				...
				case 32800:
					...
					if (PacketLength > PGNlength - 1)
					{
						...
						{
							if (ParseModID(ReceivedData[2]) == ModuleID)
							{
...
void SendLineCheck()
{
	...
	if (Ethernet.linkStatus() == LinkON)
	{
		...
		Eth_updateComm.beginPacket(Eth_ipDestination, UpdateSendPort);
		Eth_updateComm.write(data, sizeof(data));
		Eth_updateComm.endPacket();
	}
}
...
//void ReceiveAGIO() // this is the update-subnet stuff, we shouldn't need to do this
//{
//	if (Ethernet.linkStatus() == LinkON)
//	{
//		uint16_t len = Eth_updateComm.parsePacket();
//		if (len)
//		{
//			const uint8_t MaxReadBuffer = 100;  // special case, was 100 in SK21 code, hex lines probably longer
//			byte Data[MaxReadBuffer];
//			Eth_updateComm.read(Data, MaxReadBuffer);
...
//					{
//						networkAddress.ipOne = Data[7];
//						networkAddress.ipTwo = Data[8];
//						networkAddress.ipThree = Data[9];
//
//						//SaveData();
//
//						// restart the Teensy
