...
namespace Twol
{
    public sealed class Settings
    {
        ...
        private static IOSettings io_ = new IOSettings();
        public static IOSettings IO
        {
            get
            {
                return io_;
        ...
        public sealed class UserSettings
        {
            ...
            public string setDisplay_buttonOrder = "0,1,2,3,4,5,6,7,8";
        ...
        public sealed class IOSettings
        {
            ...
            public LoadResult Load()
            {
                string path = Path.Combine(RegistrySettings.IODirectory, RegistrySettings.IOFileName + ".XML");
                var result = XmlSettingsHandler.LoadXMLFile(path, this);
                if (result == LoadResult.MissingFile)
                {
                    Log.EventWriter("IO file does not exist or is Default, Default IO used");
                    RegistrySettings.Save("IOFileName", "Default");
                }
                else if (result == LoadResult.Failed)
                {
                    Log.EventWriter("IO Loaded With Error:" + result.ToString());
                }
            ...
            public void Save()
            {
                ...
                if (RegistrySettings.IOFileName != "" && RegistrySettings.IOFileName != "Default")
                    XmlSettingsHandler.SaveXMLFile(path, "IO", this);
                else
                    Log.EventWriter("Default IO Not saved to IO");
            }
